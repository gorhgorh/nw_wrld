    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <div className="px-6">
        <div className="flex flex-col gap-2 font-mono">
          <div>
            <div className="opacity-50 text-[11px] mb-1">Current Channel</div>
            <div className="text-neutral-300 text-[11px] py-2 px-3 bg-neutral-900/50 rounded">
              Channel {channelNumber}
            </div>
          </div>

          <div>
            <div className="text-[11px] relative inline-block mb-1">
              <span className="opacity-50">New Channel Number</span>
          ...
          {!config?.sequencerMode && inputType === "midi" && noteMatchMode === "exactNote" ? (
            <div>
              <div className="opacity-50 text-[11px] mb-1">Trigger Note (0–127)</div>
              <Select
                value={String(
                  (getMidiExactNoteMap(globalMappings)[String(newChannelNumber)] as
                    | number
                    | undefined) ?? 0
                )}
                onChange={(e) =>
              ...
              >
                {exactNoteOptions.map((opt) => {
                  ...
                  const usedByOtherSlot = Object.entries(exactNote).some(([s, v]) => {
